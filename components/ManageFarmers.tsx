...
const ManageFarmers: React.FC<ManageFarmersProps> = ({ onAddFarmer }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Gagal menambahkan peternak. Mungkin nama sudah ada.');
      console.error(err);
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="newFarmerName" className="block text-sm font-medium text-slate-300 mb-1">Nama Peternak Baru</label>
        <input
          id="newFarmerName"
          type="text"
          value={farmerName}
